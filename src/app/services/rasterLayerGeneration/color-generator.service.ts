...
export class ColorGeneratorService {
  ...
  constructor(private http: HttpClient, paramService: EventParamRegistrarService) {
    paramService.createParameterHook(EventParamRegistrarService.EVENT_TAGS.dataset, (dataset: any) => {
      if(dataset) {
        this.dataRange = dataset.dataRange;
        if(dataset.dataset == "tmin" || dataset.dataset == "tmax" || dataset.dataset == "tmean") {
          this.reverse = true;
        }
        else {
          this.reverse = false;
